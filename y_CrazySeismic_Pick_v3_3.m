...
% Reference:
% Yu, C., Zheng, Y., Shang, X., 2017. 
% Crazyseismic: A MATLAB GUI-Based Software Package for Passive Seismic Data Preprocessing. Seismological Research Letters 88, 410?415. doi:10.1785/0220160207
...
handle.h_ntrace_num     =   uicontrol(plot_panel,'Style','edit','String',num2str(para.n_per_frame),'callback',@Pick_callback_tracenumber,...,
    'Position',[.4 .65 .4 .04],'BackgroundColor',para.bcolor);
                    ...
                    if ~isnan(tt_tmp) % no such phase for this epi-center distance
                        if (tt_tmp-tt_ref > xlim_tmpL) && (tt_tmp-tt_ref < xlim_tmpR) % within the xlim window
                            ...
                            plot(handle.hax,[tt_tmp-tt_ref tt_tmp-tt_ref], [para.offset(j)-doff/2 para.offset(j)+doff/2],'color',para.phase_mark(kk).color,...,
                                'Linewidth',para.linewidth_show);
                            if para.phase_mark(kk).imark == 1
                                ntmp = ntmp + 1;
                                hp(ntmp) = text(tt_tmp-tt_ref, para.offset(j)-doff, regexprep(para.phase_mark(kk).name,'x',num2str(para.xdep,'%d')),'color',...,
                                    para.phase_mark(kk).color,'FontWeight','bold');
...
%% window panel functions
    function Pick_callback_window (h, dummy)
        ...
        if ~isempty(timewin_tmpL) && ~isempty(timewin_tmpR) && ~isempty(xlim_tmpL) && ~isempty(xlim_tmpR) && ~isempty(signalwin_tmpL) ...,
                && ~isempty(signalwin_tmpR) && ~isempty(noisewin_tmpL) && ~isempty(noisewin_tmpR) && ~isempty(normwin_tmpL) && ~isempty(normwin_tmpR)
...
%% pick panel functions
    ...
    function Pick_callback_align (h, dummy)
        ...
        j2 = min( j2, para.nl);
        for j =j1: j2
            [x0,y0] = curveintersect(tr(j).time, para.offset(j) + ...,
                tr(j).data*tr(j).polarity*tr(j).A0, para.xpick, para.ypick);
            if size(x0) > 0
                tr(j).dtshift = x0(1);
...
%         delete(hmsg);
    ...
    function Pick_callback_pca (h,dummy)
        ...
        for kk = 1:length(tmpind)
            ...
            fprintf(fid_pcalist_phase,'%s %f %f %f %d %s %s %f\n',outfilename, tr(tmpind(kk)).tt, tr(tmpind(kk)).tshift, tr(tmpind(kk)).tt+tr(tmpind(kk)).tshift,...,
                tr(tmpind(kk)).polarity, deblank(tr(tmpind(kk)).headers.kstnm'), deblank(tr(tmpind(kk)).headers.knetwk'), tr(tmpind(kk)).rayp);
        end
        fclose(fid_pcalist);
    ...
    function Pick_callback_stacking (h,dummy)
        ...
        for kk = 1:length(tmpind)
            ...
            imksac( tmphd, datawin_pc(:,kk)*tr(tmpind(kk)).polarity, fullfile(para.events{para.ievent},outfilename))
            fprintf(fid_pcalist,'%s\n',outfilename);
            fprintf(fid_pcalist_phase,'%s %f %f %f %d %s %s %f\n',outfilename, tr(tmpind(kk)).tt, tr(tmpind(kk)).tshift, ...,
                tr(tmpind(kk)).tt+tr(tmpind(kk)).tshift, tr(tmpind(kk)).polarity, deblank(tr(tmpind(kk)).headers.kstnm'), ...,
                deblank(tr(tmpind(kk)).headers.knetwk'), tr(tmpind(kk)).rayp);
        end
        fclose(fid_pcalist);
        fclose(fid_pcalist_phase);
...
%% I/O panel functions
    function Pick_callback_load_evlist(h,dummy)
        ...
        uicontrol(handle.h_hot);
        [templist, temppath] = uigetfile({'*.txt','Txt Files (*.txt)';'*.m;*.fig;*.mat;*.mdl','Matlab Files (*.m,*.fig,*.mat,*.mdl)';'*.*', 'All Files (*.*)'},...,
            'Load event list',para.evpathname);
        if templist == 0
            uicontrol(handle.h_hot)
        else
    ...
    function Pick_callback_load_listname (h, dummy)
        ...
        [templist, temppath] = uigetfile({'*list*', 'List Files (*list*)';'*.txt','Txt Files (*.txt)';'*.m;*.fig;*.mat;*.mdl',...,
            'Matlab Files (*.m,*.fig,*.mat,*.mdl)';'*.*', 'All Files (*.*)'}, 'Load event list',para.events{para.ievent});
...
%         fprintf(fid2,'#filename theo_tt tshift obs_tt polarity stnm netwk rayp stla stlo stel evla evlo evdp dist az baz\n');
        fprintf(fid2,'#filename theo_tt tshift obs_tt polarity stnm netwk rayp stla stlo stel evla evlo evdp dist az baz snr0 xcoeff0 ccmean\n');
        for j=1 : para.nl
            if tr(j).visible == 1
%                 fprintf(fid2,'%s %f %f %f %d %s %s %f %f %f %f %f %f %f %f %f %f\n',。。tr(j).filename, tr(j).tt, tr(j).tshift, tr(j).tt+tr(j).tshift,...,
% tr(j).polarity,tr(j).stnm,tr(j).netwk, tr(j).rayp, tr(j).headers.stla, tr(j).headers.stlo, tr(j).headers.stel, tr(j).headers.evla, tr(j).headers.evlo, ...,
% tr(j).headers.evdp, tr(j).dist, tr(j).az, tr(j).baz);
                fprintf(fid2,'%s %f %f %f %d %s %s %f %f %f %f %f %f %f %f %f %f %f %f %f\n',tr(j).filename, tr(j).tt, tr(j).tshift, tr(j).tt+tr(j).tshift, ...,
                    tr(j).polarity,tr(j).stnm,tr(j).netwk, tr(j).rayp, tr(j).headers.stla, tr(j).headers.stlo, tr(j).headers.stel, tr(j).headers.evla, ...,
                    tr(j).headers.evlo, tr(j).headers.evdp, tr(j).dist, tr(j).az, tr(j).baz, tr(j).snr0, tr(j).xcoeff0, tr(j).ccmean);
            end
        end
        fclose(fid2);
...
%         fprintf(fid2,'#filename theo_tt tshift obs_tt polarity stnm netwk rayp stla stlo stel evla evlo evdp dist az baz\n');
        ...
        for kk = 1:length(j)
            if tr(j(kk)).visible == 1
                fprintf(fid1,'%s\n',tr(j(kk)).filename);
%                 fprintf(fid2,'%s %f %f %f %d %s %s %f %f %f %f %f %f %f %f %f %f\n',tr(j(kk)).filename, tr(j(kk)).tt, tr(j(kk)).tshift, ...
% tr(j(kk)).tt+tr(j(kk)).tshift, tr(j(kk)).polarity,tr(j(kk)).stnm,tr(j(kk)).netwk, tr(j(kk)).rayp, tr(j(kk)).headers.stla, tr(j(kk)).headers.stlo, ...
% tr(j(kk)).headers.stel, tr(j(kk)).headers.evla, tr(j(kk)).headers.evlo, tr(j(kk)).headers.evdp, tr(j(kk)).dist, tr(j(kk)).az, tr(j(kk)).baz);
                fprintf(fid2,'%s %f %f %f %d %s %s %f %f %f %f %f %f %f %f %f %f %f %f\n',tr(j(kk)).filename, tr(j(kk)).tt, tr(j(kk)).tshift, ...,
                    tr(j(kk)).tt+tr(j(kk)).tshift, tr(j(kk)).polarity,tr(j(kk)).stnm,tr(j(kk)).netwk, tr(j(kk)).rayp, tr(j(kk)).headers.stla, ...,
                    tr(j(kk)).headers.stlo, tr(j(kk)).headers.stel, tr(j(kk)).headers.evla, tr(j(kk)).headers.evlo, tr(j(kk)).headers.evdp, ...,
                    tr(j(kk)).dist, tr(j(kk)).az, tr(j(kk)).baz, tr(j(kk)).snr0, tr(j(kk)).xcoeff0);
            end
        end
        fclose(fid1);
        fclose(fid2);
